...
//         let lon = placemarks?.first?.location?.coordinate.longitude {
      ...
      apiService.getJSON(urlString: "https://api.openweathermap.org/data/2.5/onecall?lat=\(self.coordinates.lat)&lon=\(self.coordinates.lon)&exclude=hourly,minutely&appid=\(self.APIKEY)&units=metric") { (result: Result<DailyForecast,ApiService.APIError>) in
          switch result {
